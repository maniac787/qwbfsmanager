...
QTime Driver::estimatedTimeForTask( qint64 x, qint64 max )
{
    static time_t start_time;
    static qint64 expected_total;
    qint64 d;
    int h, m, s;
    ...
    if ( x == max ) {
        d = qint64( time( 0 ) - start_time );
        h = int( d / 3600 );
        m = int( ( d / 60 ) % 60 );
        s = int( d % 60 );
        return QTime( qBound( 0, h, 23 ), qBound( 0, m, 59 ), qBound( 0, s, 59 ) );
    }
