...
bool DiscModel::dropMimeData( const QMimeData* data, Qt::DropAction action, int row, int column, const QModelIndex& parent )
{
    ...
    if ( data->formats().contains( WBFS_DISCS_FORMAT ) ) {
        const QWBFS::Model::DiscList parsedDiscs = QWBFS::Model::Disc::fromByteArray( data->data( WBFS_DISCS_FORMAT ) );
        discs = QSet<QWBFS::Model::Disc>( parsedDiscs.begin(), parsedDiscs.end() );
    }
        ...
        foreach ( const QUrl& url, data->urls() ) {
            ...
            if ( !file.isDir() && file.exists() ) {
                ...
                if ( suffix == QLatin1String( "wbfs" ) ) {
                    if ( Driver::wbfsFileInfo( fileName, disc ) != Driver::Ok || !disc.isValid() ) {
                        continue;
                    }
                }
                else if ( suffix == QLatin1String( "iso" ) ) {
                    disc = Driver::isoDiscInfo( fileName );
                    if ( !disc.isValid() ) {
                        continue;
                    }
                }
                else if ( mDriver && mDriver->isOpen() ) {
                    if ( mDriver->discImageInfo( fileName, disc ) != Driver::Ok ) {
                        continue;
                    }
                }
                else {
