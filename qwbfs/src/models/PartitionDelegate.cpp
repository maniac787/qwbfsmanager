...
void PartitionDelegate::paint( QPainter* painter, const QStyleOptionViewItem& _option, const QModelIndex& index ) const
{
    ...
    else if ( partition.property( pPartition::TotalSize ).toLongLong() <= 0 ) {
        // Refresh FAT/NTFS sizes from the mounted volume when udev did not provide them.
        QStringList mounts = partition.property( pPartition::MountPoints ).toStringList();
        if ( mounts.isEmpty() ) {
            const QString mount = partition.property( pPartition::MountPoints ).toString();
            if ( !mount.isEmpty() ) {
                mounts << mount;
            }
        }
        if ( mounts.isEmpty() ) {
            foreach ( const QStorageInfo& volume, QStorageInfo::mountedVolumes() ) {
                if ( volume.isValid() && volume.isReady()
                    && QString::fromUtf8( volume.device() ) == partition.devicePath() ) {
                    mounts << volume.rootPath();
                    break;
                }
            }
        }
        if ( !mounts.isEmpty() ) {
            QStorageInfo info( mounts.first() );
            if ( info.isValid() && info.isReady() && info.bytesTotal() > 0 ) {
                partition.updateSizes( info.bytesTotal(), info.bytesAvailable() );
                mModel->updatePartition( partition );
            }
        }
    }
